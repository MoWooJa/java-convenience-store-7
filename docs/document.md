...
- [ ] 결제 시스템
    - [ ] 사용자가 입력한 상품의 가격과 수량을 기반으로 최종 결제 금액 계산
        - [ ] 총 구매액 = 각 상품 별로 가격 * 수량, 프로모션 및 멤버십 할인 정책 반영애 산출
    - [ ] 구매 내역과 산출 금액 정보를 영수증으로 출력
    - [ ] 영수증 출력 후 추가 진행, 종료 선택 가능

- [ ] 재고 관리
    - [ ] 각 상품의 재고 수량을 고려하여 결제 가능 여부를 확인
    - [ ] 상품을 구매할 때마다, 결제된 수량만큼 해당 상품의 재고에서 차감하여 수량을 관리
    - [ ] 재 구매시 최신의 재고 상태 유지
- [ ] 프로모션 할인
    - [ ] 적용 기준 : 오늘 날짜가 in range 기간
    - [ ] 1+1, 2+1 2가지
    - [ ] 프로모션 재고 내에서만 적용
