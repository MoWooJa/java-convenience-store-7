...
아래 질문에 대한 최종 회고를 진행하고 소감에 구체적인 결과를 작성한다.
소감은 텍스트로 작성해야 하며 외부 링크는 허용하지 않는다.
...
- 매주 진행할 미션은 화요일 오후 3시부터 확인할 수 있으며,
- 다음 주 월요일까지 구현을 완료하여 제출해야 한다.
...
* Windows 사용자의 경우 gradlew.bat clean test 또는 .\gradlew.bat clean
* test 명령을 실행할 때 모든 테스트가 아래와 같이 통과하는지 확인한다.
  ``` 
